...
# Define the file path for the chatgpt dataset
chatgpt_file_path = os.path.join("datasets", "chatgpt.xlsx")
...
data = pd.read_excel(chatgpt_file_path)
...
results_df = pd.DataFrame(results, index=['chatgpt'])

# Save the results to an Excel file (generic path for GitHub repository)
output_file_path = os.path.join("datasets", "chatgpt_results.xlsx")
